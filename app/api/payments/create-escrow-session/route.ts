...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'down_payment' && amount < 1000) {
      console.log('down payment too small')
      return NextResponse.json(
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${process.env.NEXT_PUBLIC_API_URL}/test/payment-success?session_id={CHECKOUT_SESSION_ID}&type=escrow`,
      cancel_url: `${process.env.NEXT_PUBLIC_API_URL}/payment-cancelled?type=escrow`,
      metadata: {
        user_id: user.id,
        application_id,
        seller_id,
    ...
    console.log('created transaction', session)
